...
def set_parameters(time_window,max_ride,head_way,dwell_reduction ):
    ...
    df_parameters.loc[1,'number of routes'] = 2
...
def generate_pyomo_data(df_parameters,df_nodes,df_buses,df_tt,df_dist,dir_name,file_name):    
    ...
    sets['S'] = range(0,int(df_parameters.loc[1,'max bus number']) + 1) # set of different numbers of buses you can have on routes
...
def generate_data_file(time_window,day,max_ride,head_way,dir_name,file_name,dwell_reduction):
    ...
    # need to read in the 3 dictionaries instead no need for df_sumo_gal anymore
    ...
    # set parameters 
    df_parameters = set_parameters(time_window,max_ride,head_way,dwell_reduction) # should be ok
    ...
    df_tt = create_tt_matrix(df_sumo_tt,df_log_tt,df_nodes,df_parameters) # this should be a much simplier function
    ...
    # create data file
    generate_pyomo_data(df_parameters,df_nodes,df_buses,df_tt,df_dist,dir_name,file_name) # remove df_dist
